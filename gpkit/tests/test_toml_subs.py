...
class Aircraft(Model):
    def setup(self):
        W = Variable("W", 5000, "N", "weight")
        self.engine = Engine()
        self.cost = W
        return [W >= W, self.engine]
...
class TestSaveSubs:
    ...
    def test_save_subs_monomial_value(self):
        ...
        class Orbit(Model):
            def setup(self):
                ...
                r = Variable("r", "km")
                self.cost = r
                return [r >= r, mu >= mu], {mu: 9.0 * units("km^2/s^2")}
    ...
    def test_save_subs_writes_file(self, tmp_path):
        ...
        returned = save_subs(m, path=path)
        assert path.exists()
...
class TestLoadSubs:
    ...
    def test_load_subs_dimensioned(self):
        subs = load_subs(io.BytesIO(SIMPLE_SUBS_TOML))
        w_val = subs["Aircraft"]["W"]
        # Should be a pint Quantity whose magnitude in N is approximately 5000
        assert hasattr(w_val, "to")  # pint Quantity
        assert w_val.to("N").magnitude == pytest.approx(5000.0)
    ...
    def test_load_subs_from_path(self, tmp_path):
        ...
        p.write_bytes(SIMPLE_SUBS_TOML)
        subs = load_subs(p)
        assert "Aircraft" in subs
...
class TestApplySubs:
    ...
    def test_apply_subs_roundtrip(self):
        ...
        m = Aircraft()
        subs = load_subs(io.BytesIO(save_subs(m).encode()))
        apply_subs(m, subs)
        W_vk = self._aircraft_W_vk(m)
        assert float(m.substitutions[W_vk]) == pytest.approx(5000.0)
    ...
    def test_apply_subs_from_path(self, tmp_path):
        ...
        apply_subs(m, path)
        W_vk = self._aircraft_W_vk(m)
        assert float(m.substitutions[W_vk]) == pytest.approx(7000.0)
    ...
    def test_apply_subs_model_evolution(self):
        ...
        class Aircraft(Model):  # shadows module-level fixture in this scope
            def setup(self):
                W = Variable("W", 5000, "N", "weight")
                self.cost = W
                return [W >= W]
        ...
        with pytest.warns(UserWarning, match="Aircraft.Engine"):
            apply_subs(m_new, old_subs)

        # W (which still exists) must be applied
        W_vk = next(vk for vk in m_new.unique_varkeys if vk.name == "W")
        assert float(m_new.substitutions[W_vk]) == pytest.approx(5000.0)
