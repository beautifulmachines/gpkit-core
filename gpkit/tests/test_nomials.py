...
class TestMonomial:  # pylint: disable=unnecessary-negation,comparison-with-itself
    ...
    def test_pow_ast_exponent(self):
        ...
        assert "^2" in p2_str
        assert p2_str == "gpkit.Posynomial((x³ + x + 5)^2)"
    ...
    def test_str_with_units(self):
        ...
        assert "S" in xstr and "rho" in xstr

    def test_add(self):
        x = Variable("x")
        y = Variable("y", units="ft")
        with pytest.raises(gpkit.DimensionalityError):
            _ = x + y
    ...
    def test_eq_ne(self):
        ...
        # numeric
        assert Monomial(3) == 3
        assert Monomial(3) == Monomial(3)
        assert Monomial(3) != 2
        assert Variable("x") != 3
        assert Monomial(3) != Variable("x")
    ...
    def test_div(self):
        ...
        c = a / (0.5 * t**2 / x)
        assert c == Monomial({"x": 2, "y": -1, "t": -2}, 72)

    def test_mul(self):
        "Test monomial multiplication"
        x = Monomial({"x": 1, "y": -1}, 4)
        # test integer division
        assert x / 5 == Monomial({"x": 1, "y": -1}, 0.8)
        # divide by scalar
        assert x * 9 == Monomial({"x": 1, "y": -1}, 36)
        # divide by Monomial
        y = x * Variable("z")
        assert y == Monomial({"x": 1, "y": -1, "z": 1}, 4)
        # make sure x unchanged
        assert x == Monomial({"x": 1, "y": -1}, 4)
        # mixed new and old vars
        z = x * Monomial({"x": -1, "t": 2}, 0.5)
        assert z == Monomial({"x": 0, "y": -1, "t": 2}, 2)
...
class TestSignomial:
    ...
    def test_chop(self):
        ...
        with pytest.raises(InvalidPosynomial):
            c.chop()
        with SignomialsEnabled():
            assert -0.2 * x * y**0.78 in monomials
...
class TestPosynomial:
    ...
    def test_constraint_gen(self):
        ...
        y = Variable("y")
        p = x**2 + 2 * y * x + y**2
        assert (p <= 1).as_hmapslt1({}) == [p.hmap]
        assert (p <= x).as_hmapslt1({}) == [(p / x).hmap]
